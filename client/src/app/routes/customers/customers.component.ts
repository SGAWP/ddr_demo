...
const COLS = [
  { value: "customer_name", displayName: 'Заказчик' }
];
...
export class CustomersComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private _customersService: SprCustomersService,
  ...
  load() {
    this.loading = true;
    this.isActive = false;
    this._customersService.fetch(this.search, this.sort.active, this.sort.direction, this.pageSize, this.page).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
      this.loading = false;
      this.isActive = true;
  ...
  delete(row: Customer) {
    let dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        ...
        message: `Вы уверены что хотите удалить заказчик ${row.customer_name}?`
      }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this._customersService.delete(row.customer_id).subscribe(
