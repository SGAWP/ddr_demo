...
export class TypesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private _typesService: SprTypesService,
  ...
  load() {
    this.loading = true;
    this.isActive = false;
    this._typesService.fetch(this.search, this.sort.active, this.sort.direction, this.pageSize, this.page).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
      this.loading = false;
      this.isActive = true;
  ...
  edit(row: Type) {
    let dialogRef = this.dialog.open(EditTypeComponent, {
      width: "500px",
  ...
  delete(row: Type) {
    let dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        ...
        message: `Вы уверены что хотите удалить тип ${row.type}?`
      }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this._typesService.delete(row.type_id).subscribe(
