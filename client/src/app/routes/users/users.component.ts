...
const COLS = [
  { value: "full_name", displayName: 'ФИО' },
  { value: "username", displayName: 'Имя пользователя' },
  { value: "role_name", displayName: 'Роль' }
];
...
export class UsersComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  load() {
    this.loading = true;
    this.isActive = false;
    this._userService.fetch(this.search, this.sort.active, this.sort.direction, this.pageSize, this.page).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
      this.loading = false;
      this.isActive = true;
  ...
  delete(row: User) {
    let dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        ...
        message: `Вы уверены что хотите удалить пользователя ${row.full_name}?`
      }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this._userService.delete(row.user_id).subscribe(
