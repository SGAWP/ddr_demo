...
import { WellPurpose } from "../../shared/interfaces";
...
const COLS = [
  { value: "full_name", displayName: 'Назначение' }
];

@Component({
  selector: 'app-well-purposes',
  templateUrl: './well-purposes.component.html',
  styleUrls: ['./well-purposes.component.scss']
})
export class WellPurposesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  load() {
    this.loading = true;
    this.isActive = false;
    this._wellPurposesService.fetch(this.search, this.sort.active, this.sort.direction, this.pageSize, this.page).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
      this.loading = false;
      this.isActive = true;
  ...
  add() {
    let dialogRef = this.dialog.open(AddWellPurposeComponent, {
      width: "500px",
  ...
  edit(row: WellPurpose) {
    let dialogRef = this.dialog.open(EditWellPurposeComponent, {
      width: "500px",
  ...
  delete(row: WellPurpose) {
    let dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        title: "Подтвердите удаление",
        message: `Вы уверены что хотите удалить назначение ${row.full_name}?`
      }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this._wellPurposesService.delete(row.well_purpose_id).subscribe(
