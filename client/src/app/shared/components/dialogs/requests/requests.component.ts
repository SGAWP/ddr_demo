...
export class RequestsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  load() {
    this.loading = true;
    this.isActive = false;
    this._requestsService.getAllIsActive(this.search, this.sort.active, this.sort.direction, this.pageSize, this.page).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
      this.loading = false;
      this.isActive = true;
